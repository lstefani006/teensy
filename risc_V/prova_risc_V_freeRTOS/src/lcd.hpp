...
class Draw_ShowString : public Draw_Base
{
public:
    Draw_ShowString(u16 x, u16 y, const u8 *p, u16 color)
        :x(x), y(y), color(color)
    {
        this->p = (u8 *)strdup((char *)p);
    }
    ~Draw_ShowString() override { if (p) free(p); }
    void Draw() override { LCD_ShowString(x, y, p, color); }
...
class Draw_Clear : public Draw_Base
 {
public:
    Draw_Clear(u16 color) : color(color) {}
    void Draw() override { LCD_Clear(color); }

private:
    u16 color;
};
class Draw_Line : public Draw_Base
{
public:
    Draw_Line(u16 x1, u16 y1, u16 x2, u16 y2, u16 color) : x1(x1), y1(y1), x2(x2), y2(y2), color(color) {}
    void Draw() override { LCD_DrawLine(x1, y1, x2, y2, color); }

private:
    u16 x1, y1, x2, y2, color;
};
class Draw_Rectangle : public Draw_Base
{
public:
    Draw_Rectangle(u16 x1, u16 y1, u16 x2, u16 y2, u16 color) : x1(x1), y1(y1), x2(x2), y2(y2), color(color) {}
    void Draw() override { LCD_DrawRectangle(x1, y1, x2, y2, color); }

private:
    u16 x1, y1, x2, y2, color;
};
class Draw_List : public Draw_Base
{
public:
    Draw_List() {}
    void Add(Draw_Base *p) { _b.push_back(p); }
    ~Draw_List() override
    {
        for (auto &b : _b)
            delete b;
