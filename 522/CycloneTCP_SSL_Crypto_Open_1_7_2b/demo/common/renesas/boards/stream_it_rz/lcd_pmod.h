...
/* Pin defines common to both displays */
   ...
   /* Display enable pin */
   #define ENABLE_PIN_BIT            ((uint16_t) (1 << 4))
#elif defined(USE_PMOD2)
   /* Display/enable pin */
   #define ENABLE_PIN                (GPIO.P1)
   /* Display enable pin */
   #define ENABLE_PIN_BIT            ((uint16_t) (1 << 5))
#else
   /* Display/enable pin */
   #define ENABLE_PIN                (GPIO.P6)
   /* Display enable pin */
   #define ENABLE_PIN_BIT            ((uint16_t) (1 << 13))
#endif

#if defined(USE_PMOD1) || defined(USE_PMOD2)
   /* Vbat enable pin */
   #define DATA_CMD_PIN              (GPIO.P4)
   #define DATA_CMD_PIN_BIT          ((uint16_t) (1 << 15))

   /* Vdd enable pin */
   #define BL_ENABLE_PIN             (GPIO.P3)
   #define BL_ENABLE_PIN_BIT         ((uint16_t) (1 << 7))
#else
   /* Vbat enable pin */
   #define DATA_CMD_PIN              (GPIO.P9)
   #define DATA_CMD_PIN_BIT          ((uint16_t) (1 << 5))

   /* Reset pin */
   #define LCD_RST_PIN               (GPIO.P3)
   #define LCD_RST_PIN_BIT           ((uint16_t) (1 << 15))
#endif
