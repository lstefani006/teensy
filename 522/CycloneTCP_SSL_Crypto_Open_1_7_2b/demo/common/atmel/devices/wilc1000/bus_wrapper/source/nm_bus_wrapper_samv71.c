...
static sint8 spi_rw(uint8 *pu8Mosi, uint8 *pu8Miso, uint16 u16Sz)
{
   ...
   //Set source address
   XDMAC->XDMAC_CHID[1].XDMAC_CSA = (uint32_t) pu8Mosi;
   ...
   if(pu8Miso != NULL)
   {
      ...
      //Set destination address
      XDMAC->XDMAC_CHID[2].XDMAC_CDA = (uint32_t) pu8Miso;
      //Set the number of data to be transferred
   ...
   {
      //Enable channel 1 interrupt enable bit
      XDMAC->XDMAC_GIE = XDMAC_GIE_IE1;
      //Enable channel 1 (SPI0 TX)
